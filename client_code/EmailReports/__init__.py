    ...
    try:
      ...
      for row in self.user_values:
        for col in duration_columns:
          ...
          row[col_index] = int(
            row[col_index] // 60000
          )  # Ensure integer division and convert ms to minutes
      ...
      self.data_column_selector.items = [
        (col, col) for col in numeric_columns if isinstance(col, str)
      ]
      ...
      y_column = self.data_column_selector.selected_value or (
        "volume" if "volume" in numeric_columns else numeric_columns[0]
      )
  ...
  def _update_plot(self, y_column):
    ...
    try:
      ...
      for row in self.user_values:
        ...
        report_date = row[report_date_index].strftime(
          "%Y-%m-%d"
